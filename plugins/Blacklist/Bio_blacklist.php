<?php
/**
 * Data class for bio blacklisting
 *
 * PHP version 5
 *
 * @category Data
 ...
 * @see      DB_DataObject
 */
class Bio_blacklist extends Memcached_DataObject
{
    public $__table = 'bio_blacklist'; // table name
     ...
     *
     * @return Bio_blacklist object found, or null for no hits
     *
     */
    function staticGet($k, $v=null)
    {
        return Memcached_DataObject::staticGet('Bio_blacklist', $k, $v);
    }
    ...
    {
        $patterns = self::cacheGet('bio_blacklist:patterns');

        if ($patterns === false) {

            $patterns = array();

            $nb = new Bio_blacklist();
            ...
            self::cacheSet('bio_blacklist:patterns', $patterns);
        }
    ...
    static function saveNew($newPatterns)
    {
        ...
        foreach ($toDelete as $pattern) {
            $nb = Bio_blacklist::staticGet('pattern', $pattern);
            if (!empty($nb)) {
                $nb->delete();
            }
        }
        ...
        self::blow('bio_blacklist:patterns');
    }

    static function ensurePattern($pattern)
    {
        $hb = Bio_blacklist::staticGet('pattern', $pattern);

        if (empty($nb)) {
            $hb = new Bio_blacklist();
            $hb->pattern = $pattern;
